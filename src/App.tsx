...
export default function App() {
  ...
  const [dvdView, setDvdView] = useState<View>('lesson');
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <main id="main-content" className="flex-1 overflow-hidden min-w-0 flex flex-col" tabIndex={-1}>
        ...
        <header className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 sticky top-0 z-10 flex-shrink-0">
          ...
          {(currentModule === 'company' ? currentView : dvdView) === 'lesson' && (
          ...
          <div className="px-4 sm:px-6 py-3 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              {/* View switcher */}
              {(() => {
                const activeView = currentModule === 'company' ? currentView : dvdView;
                const setView = currentModule === 'company' ? setCurrentView : setDvdView;
                const activeColor = currentModule === 'dvd' ? 'text-violet-600 dark:text-violet-400' : 'text-indigo-600 dark:text-indigo-400';
                return (
                  <div className="flex bg-gray-100 dark:bg-gray-700 rounded-lg p-1">
                    <button
                      onClick={() => setView('lesson')}
                      className={`px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${activeView === 'lesson' ? `bg-white dark:bg-gray-600 ${activeColor} shadow-sm` : 'text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200'}`}
                    >
                    ...
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${activeView === 'playground' ? `bg-white dark:bg-gray-600 ${activeColor} shadow-sm` : 'text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200'}`}
                    >
                      <span className="flex items-center gap-1.5">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
